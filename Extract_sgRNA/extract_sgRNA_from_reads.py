...
def get_read_sequence(fastq_iter1, fastq_iter2):
    for (x1,y1,z1), (x2,y2,z2) in zip(fastq_iter1, fastq_iter2):
        yield y1,y2
...
def compare_with_known_list(obs_sequences, known_barcodes_regex, known_sgRNAs_regex):

    barcode_match = list_compare_helper(obs_sequences[0], known_barcodes_regex)
    sgRNA_match   = list_compare_helper(obs_sequences[1], known_sgRNAs_regex)
    
    return([barcode_match, sgRNA_match])


def list_compare_helper(sequence, known_list_regex):
